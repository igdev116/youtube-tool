...
let YoutubeChannelController = class YoutubeChannelController {
    ...
    async addChannelToGroup(channelDbId, groupId, req) {
        const userId = req.user.sub;
        const channel = await this.channelService.addChannelToGroup(userId, channelDbId, groupId);
        if (!channel) {
            return {
                success: false,
                statusCode: 404,
    ...
    async removeChannelFromGroup(channelDbId, groupId, req) {
        const userId = req.user.sub;
        const channel = await this.channelService.removeChannelFromGroup(userId, channelDbId, groupId);
        if (!channel) {
            return {
                success: false,
                statusCode: 404,
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], YoutubeChannelController.prototype, "addChannelToGroup", null);
__decorate([
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
