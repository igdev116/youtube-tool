...
let YoutubeChannelService = YoutubeChannelService_1 = class YoutubeChannelService {
    ...
    async addChannelToGroup(userId, channelDbId, groupId) {
        const userObjectId = new mongoose_2.Types.ObjectId(userId);
        const groupObjectId = new mongoose_2.Types.ObjectId(groupId);
        const channel = await this.channelModel
            .findOneAndUpdate({ _id: channelDbId, user: userObjectId }, { $addToSet: { groups: groupObjectId } }, { new: true })
            .populate('groups', '_id name');
        if (!channel)
            return null;
        return channel;
    }
    async removeChannelFromGroup(userId, channelDbId, groupId) {
        const userObjectId = new mongoose_2.Types.ObjectId(userId);
        const groupObjectId = new mongoose_2.Types.ObjectId(groupId);
        const channel = await this.channelModel
            .findOneAndUpdate({ _id: channelDbId, user: userObjectId }, { $pull: { groups: groupObjectId } }, { new: true })
            .populate('groups', '_id name');
        if (!channel)
            return null;
        return channel;
